...
from ideal_functions.datasets import IdealDataSet, TestDataSet, TrainingDataSet
...
def run(data_dir: str | Path, output_dir: str | Path) -> None:
    ...
    training = TrainingDataSet(data_dir / "train.csv").frame

    ideal = IdealDataSet(data_dir / "ideal.csv").frame

    test = TestDataSet(data_dir / "test.csv").frame
    ...
    database_path = output_dir / "ideal_functions.sqlite"

    with SQLiteRepository(database_path) as repository:
        repository.write_sources(training, ideal, test)

        repository.write_mappings(mappings)

        selection_data = pd.DataFrame(selection.__dict__ for selection in selections)

        repository.write_selections(selection_data)

    visualization_path = output_dir / "visualization.html"

    create_visualization(training, ideal, mappings, selections, visualization_path)
    ...
    assigned_count = mappings["ideal_function"].notna().sum()

    print(f"Mapped {assigned_count} " f"of {len(test)} test points.")
